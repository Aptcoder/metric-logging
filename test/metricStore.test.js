...
describe('Store for management the metrics', () => {
  it('Should read correct data from store', async () => {
    const data = await MetricStore.readStore('./test/mock/metric-store.json');
    expect(data).to.eql(store);
  });

  // test write to store
  it('Should write correctly to the store when key already exists', async () => {
    await MetricStore.writeToStore('asset_value', 348, './test/mock/metric-store.json');
    const data = await MetricStore.readStore('./test/mock/metric-store.json');
  ...
  it('Should write correctly to the store when key does not exist', async () => {
    await MetricStore.writeToStore('new_value', 348, './test/mock/metric-store.json');
    const data = await MetricStore.readStore('./test/mock/metric-store.json');
  ...
  it('Should get correct sum from store', async () => {
    const sum = await MetricStore.getSumForKey('asset_value', './test/mock/metric-store.json');
